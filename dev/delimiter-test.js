import { pathContours, translateContours } from "../src/opentype-util";
import { LitElement, html, css } from 'lit-element';
import '@polymer/paper-slider/paper-slider.js';
import { delimiterFontData, createDelimiter } from "../src/create-delimiter";
...
class DelimiterByHeightTest extends LitElement {
	...
	_renderCanvas(ctx, canvas){
		...
		const heights = [0.5, 0.6, 0.9, 1.2, 1.5];
...
class ShapeEvolutionInspector extends LitElement {
	static get styles() {
		return css`
			:host {
				display: block;
	...
	_controlsTemplate(){
		return html`
			<div class="controls">
				...
				<paper-slider 
					pin min="0" max="4" step="1" value=${this.sizeIndex}
					@immediate-value-change=${e => this.sizeIndex = parseInt(e.srcElement.immediateValue)}
				></paper-slider>
			</div>
		`;
	}
	...
	drawBoundingBox(ctx, bbox){
		ctx.rect(bbox.xMin, bbox.yMin - 0.25, bbox.xMax - bbox.xMin, bbox.yMax - bbox.yMin);
		ctx.lineWidth = 0.02;
		ctx.stroke();
	}
	_renderCanvas(ctx, canvas){
		...
		ctx.setTransform(scale, 0, 0, -scale, 0, canvas.height / 2);
		ctx.lineWidth = 0.02;
		drawHorizontalLine(ctx, 0, canvas.width / scale, 0);

		const unicode = 40;
		const glyphEntry = delimiterFontData[unicode][this.sizeIndex];
		const glyphHeight = glyphEntry.bbox.yMax - 0.25;
		const normScale = 3 / glyphHeight;
		const contours = translateContours([0, -0.25], glyphEntry.contours);
...
customElements.define("shape-evolution-inspector", ShapeEvolutionInspector);
